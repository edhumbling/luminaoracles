...
void main(){gl_Position=position;}`;
  ...
  init() {
    ...
    // Store uniform locations in a typed object
  ...
  render(now = 0) {
    ...
    gl.uniform2f(this.uniformLocations!.move, ...this.mouseMove);
    gl.uniform2f(this.uniformLocations!.touch, ...this.mouseCoords);
...
const useShaderBackground = () => {
  ...
  const animationFrameRef = useRef<number | undefined>(undefined); // Fix undefined vs null
...
// Export standalone shader component for general usage
export const AnimatedShader: React.FC<{ className?: string }> = ({ className }) => {
  const canvasRef = useShaderBackground();

  return (
    <canvas
      ref={canvasRef}
      className={`block w-full h-full object-cover touch-none ${className || ''}`}
      style={{ background: 'black' }}
    />
  );
};
